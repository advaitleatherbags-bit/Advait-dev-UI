...
function PaymentFailureContent() {
  ...
  const retryPayment = async () => {
    ...
    if (
      ...
      setError(
        'The payment callback did not include a valid order reference. Please return to your cart and start checkout again.'
      )
      return
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE}/orders/${orderId}/retry-payment`,
        {
          method: 'POST',
          headers: {
      ...
      >
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 text-center">
          <p className="text-xs text-gray-400">
            Your card has not been charged for this transaction
          </p>
        </div>
      </motion.div>
    </div>
